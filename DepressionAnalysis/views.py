...
def index(request):
    ...
    auth_manager = spotipy.oauth2.SpotifyOAuth(redirect_uri=SPOTIPY_REDIRECT_URI,client_id=SPOTIPY_CLIENT_ID,client_secret=SPORIPY_CLIENT_SECRET,scope = "user-top-read user-library-read", cache_handler=cache_handle,show_dialog=True)
    ...
    context = {'results' : spotifee, 'cover_art' : spotifee[4], 'c2' : spotifee[8], "saved" : spotifee[9], 'score' : spotifee[0], 'min_name' : spotifee[1], "min_artist" : spotifee[2], "max_name" : spotifee[5], "max_artist" : spotifee[6]}
